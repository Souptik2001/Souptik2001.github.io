...
function setup() {
    ...
    var mesh = [];
    var n_rows = Math.floor(windowHeight / (2 * gridDim));
    var n_cols = Math.floor(windowWidth / gridDim);
    stroke(225);
    // line(20, 20, 80, 20);
    // Drawing the grid
    // Horizontal Lines

    for (var i = 0; i < Math.floor(windowHeight / 2) / gridDim; i++) {
        line(0, Math.floor((windowHeight / 2)) + (i * gridDim), windowWidth, Math.floor(windowHeight / 2) + (i * gridDim));
    }
    // Vertical Lines
    for (i = 0; i < (windowWidth) / gridDim; i++) {
        line(i * gridDim, Math.floor(windowHeight / 2), i * gridDim, windowHeight);
    }
    noLoop();
}
...
function mark_wall(x, y) {
    ...
    fill(205, 123, 212);
    stroke(0, 0, 0);
    rect(x * gridDim, (windowHeight / 2) + (y * gridDim), gridDim, gridDim);
    pop();
}
