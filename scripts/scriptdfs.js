...
async function dijksta(x, y, xe, ye) {
    ...
    // }
    var popping;
    while (open_stack.length >= 1) {
        console.log("going");
        popping = open_stack.pop();
        visited.push(popping);
        if (popping == pointe) {
            var prev_cell = table[1][popping];
            while (prev_cell != point) {
                mark_path(Math.floor(prev_cell % n_cols), Math.floor(prev_cell / n_cols));
                prev_cell = table[1][prev_cell];
                await sleep(6);
            }
            break;
        }
        mark_cell(Math.floor(popping % n_cols), Math.floor(popping / n_cols));
        if (went(visited, popping + 1) == 0 && went(open_stack, popping + 1) == 0 && (popping + 1) % n_cols < n_cols + 1) {
            console.log("yo1");
            open_stack.push(popping + 1);
            table[1][popping + 1] = popping;
        }
        await sleep(2);
        if (went(visited, popping - 1) == 0 && went(open_stack, popping - 1) == 0 && (popping - 1) % n_cols > -1) {
            console.log("yo2");
            open_stack.push(popping - 1);
            table[1][popping - 1] = popping;
        }
        await sleep(2);
        if (went(visited, popping + n_cols) == 0 && went(open_stack, popping + n_cols) == 0 && Math.floor((popping + n_cols) / n_cols) < n_rows + 1) {
            console.log("yo3");
            open_stack.push(popping + n_cols);
            table[1][popping + n_cols] = popping;
        }
        await sleep(2);
        if (went(visited, popping - n_cols) == 0 && went(open_stack, popping - n_cols) == 0 && Math.floor((popping - n_cols) / n_cols) > -1) {
            console.log("yo4");
            open_stack.push(popping - n_cols);
            table[1][popping - n_cols] = popping;
        }
